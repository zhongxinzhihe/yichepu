...
class Goods extends MobileBase {
    ...
    public function goodsList(){
        ...
        if($_SESSION['shop_type']==1){
           $filter_goods_id = M('goods')->where(array('shop_id'=>$_SESSION['shop_id']))->where("is_on_sale=1")->where("cat_id", "in" ,implode(',', $cat_id_arr))->cache(true)->getField("goods_id",true); 
        }else{
    ...
    public function goodsInfo(){
        ...
        if($goods['prom_type'] == 1)
        {
            ...
            if(!empty($goods['market_price'])&&is_numeric($goods['market_price'])&&$goods['market_price']!=0){
...
//        $collect = M('goods_collect')->where("goods_id=$goods_id and user_id=$user_id")->count();
        ...
        //所有商品、最新、热卖
        $infos['all'] = M('goods')->where(array('shop_id'=>$goods['shop_id']))->count();
        $infos['new'] = M('goods')->where(array('shop_id'=>$goods['shop_id'],'is_new'=>1))->count();
        $infos['hot'] = M('goods')->where(array('shop_id'=>$goods['shop_id'],'is_hot'=>1))->count();

   ...
   {                
    ...
    public function add_order()
    {
        ...
        if(IS_POST)
        {
           ...
           $order['order_amount']   = $result['result']['order_amount']; // 应付金额
           $order['total_amount']   = $result['result']['total_amount']; // 订单总价
           $_SESSION['type']==1 ? $order['shop_id']=$_SESSION['admin_id']:false;
    ...
    {
         $_SESSION['type']==1? $brandList =M("brand")->where(array('shop_id'=>$_SESSION['admin_id']))->select():$brandList = M("brand")->select();
    	 $_SESSION['type']==1? $categoryList =M("goods_category")->where(array('shop_id'=>$_SESSION['admin_id']))->select():$categoryList =  M("goods_category")->select();

...
function ToUrlParams($values)
  ...
  function shunfuPay($data,$typeNo,$openid='',$url='http://pay.shunfu-pay.cn/shunfupay-admin/api/pay/doPay.html')
      {
        $url= $url;
        $mchNo = C('shunfuMchNo');  
        $key = C('shunfuKey');
        ...
        // $mchNo = 'shunfupay002';  
        // $key = '1f6304a9bc9467c79ecd4525edab35fa';
        $extendParams = array('share_gid'=>$_SESSION['share_gid'],'share_uid'=>$_SESSION['share_uid'],'user_id'=>$_SESSION['user']['user_id']);
        $post_data=array(
            'mchNo' =>$mchNo,
      ...
      function shunfuRefund()
      {
        // $url= $url;
        $mchNo = C('shunfuMchNo');  
        $key = C('shunfuKey');
            // var_dump($key);die();
        $url= "http://pay.shunfu-pay.cn/shunfupay-admin/api/refund/doRefund.html";
        // $mchNo = 'shunfupay002';  
        // $key = '1f6304a9bc9467c79ecd4525edab35fa';
        $post_data=array(
            'mchNo' =>$mchNo,
            'gwTradeNo'=>'2017091815230500145894431',
            'orderNo' =>  '201709181522572396',
          
            'refundNo' =>  time().'123456',
            'refundAmount' =>  '0.01',
            'timeStamp' =>  time(),
      ...
      function jsonToStr($jsonstring='')
      {
         ...
         foreach ($arr as $key => $value) {
            $str .=$value[0].'：'.$value[2].'次,'; 
         }
         return $str;

      }

      function set_meals($jsonstring)
      {
           $str = '';
           $arr =  json_decode($jsonstring);
           foreach ($arr as $key => $value) {
              $data = M('SetMeal')->where(array('id'=>$value))->find();
              $str .="<a href='/index.php/Admin/Card/add_set_meal/id/$data[id]'>".$data[name].'</a>'; 
           }
           return $str;
      }
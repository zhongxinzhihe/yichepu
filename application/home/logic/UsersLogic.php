...
class UsersLogic extends Model
{
    ...
    public function login($username,$password){
        ...
        if(!$username || !$password)
         ...
         $user = M('users')->where("mobile",$username)->whereOr('email',$username)->find();
           // $user = M('users')->where(array('mobile'=>$username,'oauth'=>array('eq','')))->whereOr(array('email'=>$username,'oauth'=>array('eq','')))->find();
        if(!$user){
    ...
    public function thirdLogin($data=array()){
        ...
        if(isset($data['unionid'])){
        	...
        	$user = get_user_info($data['unionid'],4,$oauth,$openid);
        }else{
        ...
        if(!$user){
            ...
            $user['token'] = md5(time().mt_rand(1,999999999));
            // M('users')->where("user_id", $user['user_id'])->save(array('token'=>$user['token'],'last_login'=>time(),'head_pic'=>$data['head_pic']));
            $savedata = array();
            $savedata['token'] = $user['token'];
            $savedata['last_login'] = time();
            $savedata['head_pic'] = $data['head_pic'];
            if ($data['type']=='1') {//1是微信公众号登陆
               $savedata['openid'] = $data['openid'];
            }
            
            if(isset($data['unionid'])) $savedata['unionid'] = $data['unionid'];
            M('users')->where("user_id", $user['user_id'])->save($savedata);
        }
    ...
    public function get_order_goods($order_id){
        $sql = "SELECT og.*,ogt.start_time,ogt.end_time FROM __PREFIX__order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id LEFT JOIN __PREFIX__ogtime ogt ON ogt.rec_id = og.rec_id WHERE order_id = :order_id";
        $bind['order_id'] = $order_id;
        $goods_list = DB::query($sql,$bind);
        foreach ($goods_list as $key => $value) {
            $check_logs = M('check_order')->where(array('order_sn'=>$value['goods_osn']))->field('shop_name,add_time')->select();
            $value['check_logs'] =$check_logs;
...
//        $type = I('get.type',0);
    ...
    public function send_email_code($email)
    {
        ...
        $res = send_email($email,'三品車','三品車验证码:'.$email_code);
        if ($res) {
            $arr = array('code'=>$email_code,'time'=>time()+120,'sender'=>$email);
            session('validate_code',$arr);

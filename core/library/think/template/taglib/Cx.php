...
class Cx extends Taglib
{
    ...
    protected $tags = [
        ...
        'imshop'     => ['attr'=>'sql,key,item,result_name','close'=>1,'level'=>3], // imshop sql 万能标签
        'adv'        => ['attr'=>'limit,order,where,item,sid','close'=>1],
    ];
    
	
	public function tagIMshop($tag, $content)
        {
	...
	public function tagAdv($tag, $content)
        ...
        $sid  =  !empty($tag['sid']) ? $tag['sid'] : '0'; 
        $position  =  !empty($tag['position']) ? $tag['position'] : '0';   
        $str = '<?php ';
        $str .= '$pid ='.$pid.';';
        $str .= '$sid ='.$sid.';';
        $str .= '$position ='.$position.';';
        $str .= '$ad_position = M("ad_position")->where("shop_id=$sid")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");';

        $str .= '$result = M("ad")->where("pid=$pid and shop_id=$sid and position_type=$position and enabled = 1 and start_time < '.strtotime(date('Y-m-d H:00:00')).' and end_time > '.strtotime(date('Y-m-d H:00:00')).' ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("'.$limit.'")->select();';
         // $str .= 'var_dump($result);die();';
        $str .= '

...
function swichTableValUsed(table,id_name,id_value,field,obj)
{	
		var url = "";
		var value=0;
         var soldprice;
		 if($(obj).is(':checked')) // 图片点击是否操作
		 {          
				var value = 1;
			 ...
			 layer.open({
  				type:0,
  				title :"请输入实际销售价格",
  				content: '<input type="text" name="soldprice" placeholder="0" style="width: 50%;text-align: center;" id="soldprice">',
  				closeBtn: 1,
  				move: false,
  				cancel:function () {
  				...
  				yes:function(index){
  					...
  					var url = "/index.php?m=Admin&c=Index&a=changeTableVal&table="+table+"&id_name="+id_name+"&id_value="+id_value+"&field="+field+'&value='+value+"&sold_price="+$("#soldprice").val();
  					soldprice = parseInt($("#soldprice").val());
  					$.ajax({
  						url:url,
  						success: function(data){
  							layer.msg('更新成功', {icon: 1});  
...
function swichTableVal(table,id_name,id_value,field,obj)
{	
		var src = "";
		 if($(obj).is(':checked')) // 图片点击是否操作
		 {          
